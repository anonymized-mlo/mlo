...
if __name__ == "__main__":
    ...
    assert(len(X_out_train_liwc) == len(X_in_train_liwc))
    assert(len(X_out_valid_liwc) == len(X_in_valid_liwc))
    assert(len(X_out_test_liwc) == len(X_in_test_liwc))

    X_train = []
    for i in range(len(X_out_train_liwc)):
        x_out = X_out_train_liwc[i].tolist()
        x_in = X_in_train_liwc[i].tolist()
        x = x_out + x_in
        X_train.append(x)

    X_valid = []
    for i in range(len(X_out_valid_liwc)):
        x_out = X_out_valid_liwc[i].tolist()
        x_in = X_in_valid_liwc[i].tolist()
        x = x_out + x_in
        X_valid.append(x)

    X_test = []
    for i in range(len(X_out_test_liwc)):
        x_out = X_out_test_liwc[i].tolist()
        x_in = X_in_test_liwc[i].tolist()
        x = x_out + x_in
        X_test.append(x)
    ...
    with open(DATA_DIR + 'X_train_liwc_xgboost_one_hot.pkl', 'wb') as f:
        pickle.dump(one_hot_X_train_leaves, f)
    with open(DATA_DIR + 'X_valid_liwc_xgboost_one_hot.pkl', 'wb') as f:
        pickle.dump(one_hot_X_valid_leaves, f)
    with open(DATA_DIR + 'X_test_liwc_xgboost_one_hot.pkl', 'wb') as f:
