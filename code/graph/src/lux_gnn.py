...
from lux_conv import SignedSAGEConvolutionBase, SignedSAGEConvolutionDeep
from lux_conv import ListModule
from param_parser import parameter_parser

args = parameter_parser()
edge_features_count = pd.read_csv(args.edge_fe_path, nrows=0).shape[1]
...
class SignedGraphConvolutionalNetwork(torch.nn.Module):
    ...
    def setup_layers(self):
        ...
        for i in range(1, self.layers): # different for deep and base - effects weight params
            self.positive_aggregators.append(SignedSAGEConvolutionDeep((4*self.neurons[i-1]),
    ...
    def forward(self, positive_edges, negative_edges, target):
        """
        self.z: Hidden node representations.
        """
        ...
        for i in range(1, self.layers):
            ...
            self.h_pos.append(torch.tanh(po)) # add relu here
...
class LuxTrainer(object):
    ...
    def create_and_train_model(self):
        ...
        for epoch in self.epochs:
            ...
            if(self.args.edge_features_incls):
                loss, _,  = self.model(self.positive_edges, self.negative_edges, self.y) #when parameterised call to class is made, __call__ is called implicitely, which calls forward()
            if(not self.args.edge_features_incls):
                loss, _,  = self.model(self.positive_edges, self.negative_edges, self.y)
            loss.backward()
            self.epochs.set_description("GNN (Loss=%g)" % round(loss.item(), 4))
            self.optimizer.step()
