...
from utils import fa, read_edge_list, sign_prediction
...
class SignedLatentFactorModel(object):
    ...
    def setup(self):
        ...
        self.train_edges = [[x[0], x[1], x[2]] for x in self.edges if x[3]=='train']
        self.test_edges = [[x[0], x[1], x[2]] for x in self.edges if x[3]=='test']
    ...
    def fit(self):
        ...
        for epoch in range(self.args.epochs+1):
            ...
            learning_rate = self.args.learning_rate * (self.args.epochs - epoch) / self.args.epochs #decaying rate
            for u in nodes:
                pbar.update(1)
                out_p_g = np.zeros(self.args.k1) # initialize as all zeros
                ...
                for succ in succs:
                    """fa is the activation function, but what is this activation function, it is """
                    e_p = fa(self.W_out_p[u] @ self.W_in_p[succ], self.args) # @ here is the matrix multiplier, returns in the range 0 ~ 1
    ...
    def save_emb(self):
        ...
        np.save(self.args.outward_embedding_path+'_'+str(self.args.k1)+'_'+str(self.args.epochs), W_out)
        np.save(self.args.inward_embedding_path+'_'+str(self.args.k1)+'_'+str(self.args.epochs), W_in)
